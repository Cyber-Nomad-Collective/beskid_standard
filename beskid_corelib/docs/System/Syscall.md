`System.Syscall` is the low-level surface for **descriptor-oriented** I/O.
...
- **`System.Syscall.StandardStream`** — `Stdin`, `Stdout`, `Stderr` enum
- **`System.Syscall.Descriptor`** — `Standard(StandardStream)` or `Raw(i64)` descriptor selector
- **`System.Syscall.ReadLimit`** — `UpTo(i64)` or `Default` read-size selector
- **`System.Syscall.SyscallError`** — `InvalidFd`, `UnsupportedReadFd`, `InvalidReadLimit`, `IoFailure`
- **`System.Syscall.WriteRequest`** — typed write DTO (`descriptor` + `data`)
- **`System.Syscall.ReadRequest`** — typed read DTO (`descriptor` + `limit`)

## Usage examples

```beskid
Core.Results.Result<i64, System.Syscall.SyscallError> out =
...
```beskid
Core.Results.Result<string, System.Syscall.SyscallError> text =
...
- One primary type per file under **`System/Syscall/`** keeps generated docs and diffs readable.
