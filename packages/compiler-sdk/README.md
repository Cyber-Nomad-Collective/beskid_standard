...
- `src/Beskid/Compiler/Syntax.bd` — thin entry (`pub mod …Nodes`, `SyntaxFacadeVersion`); no `ReflectStub` here (other hubs like `Query.bd` may still define a local stub for stitched enums).
- `src/Beskid/Compiler/Syntax/Nodes.bd` — barrel that re-exports one module per AST node type and per generated list/optional helper.
